...
class AFI_Elementor extends \ElementorPro\Modules\Forms\Classes\Action_Base {
    ...
    public function run( $record, $ajax_handler ) {
        ...
        $ef1_records = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}adfoin_integration WHERE status = 1 AND form_provider = 'elementorpro'", ARRAY_A );
        $ef2_records = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}adfoin_integration WHERE status = 1 AND form_provider = 'elementorpro2' AND form_id = 1", ARRAY_A );
        $saved_records = array_merge( $ef1_records, $ef2_records );
