...
/**
 * Plugin Name: Advanced Form Integration Professional
...
if ( function_exists( 'adfoin_fs' ) ) {
    adfoin_fs()->set_basename( true, __FILE__ );
} else {
    if ( !function_exists( 'adfoin_fs' ) ) {
        ...
        public function includes() {
            ...
            if ( adfoin_fs()->is__premium_only() ) {
                if ( adfoin_fs()->is_plan( 'professional', true ) ) {
                    ...
                    include ADVANCED_FORM_INTEGRATION_PRO . '/elementorpro2/elementorpro2.php';
                    include ADVANCED_FORM_INTEGRATION_PRO . '/webhooksinbound/webhooksinbound.php';
                    foreach ( $platform_settings as $platform => $value ) {
                        if ( true == $value ) {
                            if ( file_exists( ADVANCED_FORM_INTEGRATION_PRO . "/{$platform}pro/{$platform}pro.php" ) ) {
                                include ADVANCED_FORM_INTEGRATION_PRO . "/{$platform}pro/{$platform}pro.php";
                            }
                        }
                    }
                }
