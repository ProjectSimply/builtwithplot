...
function adfoin_amelia_get_form_fields( $form_provider, $form_id ) {
    ...
    if( 'appointmentBookingRegular' == $form_id
    || 'appointmentBookingTimeUpdated' == $form_id
    || 'appointmentBookingCanceled' == $form_id
    || 'appointmentBookingStatusUpdated' == $form_id ) {
    ...
    if( 'eventBookingRegular' == $form_id
    || 'eventBookingTimeUpdated' == $form_id
    || 'eventBookingCanceled' == $form_id
    || 'eventBookingStatusUpdated' == $form_id ) {
    ...
    if ( adfoin_fs()->is__premium_only() ) {
        if ( adfoin_fs()->is_plan( 'professional', true ) ) {
            ...
            $custom_fields = array();
            $results       = $wpdb->get_col( "SELECT label FROM {$wpdb->prefix}amelia_custom_fields" );
...
add_action( 'AmeliaBookingAddedBeforeNotify', 'adfoin_amelia_booking_added', 10, 2 );
add_action( 'AmeliaBookingTimeUpdated', 'adfoin_amelia_booking_time_updated', 10, 3 );
add_action( 'AmeliaBookingCanceled', 'adfoin_amelia_booking_canceled', 10, 3 );
add_action( 'AmeliaBookingStatusUpdated', 'adfoin_amelia_booking_status_updated', 10, 3 );
                ...
                foreach( $custom_fields as $custom_field ) {
                    if( isset( $custom_field['value'] ) && $custom_field['value'] ) {
                        ...
                        if( 'file' == $custom_field['type'] ) {
                            ...
                            $filepath   = rtrim( $upload_dir['baseurl'], '/' ) . '/amelia/files/' . $booking['id'] . '_' . $custom_field['value'][0]['fileName'];
        
                            $posted_data[$custom_field['label']] = $filepath;
        
                            continue;
                        }
        
                        $posted_data[$custom_field['label']] = $custom_field['value'];
                    }
                }
            }
        }
