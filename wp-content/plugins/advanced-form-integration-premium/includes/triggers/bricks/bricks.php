...
function adfoin_bricks_get_forms( $form_provider ) {
    ...
    $posts = get_posts( array(
        'post_type' => array( 'post', 'page', 'products', 'stories', 'insights', 'events', 'industries', 'podcasts', 'product', 'story', 'insight', 'event', 'industry', 'podcast', 'bricks_template' ),
        'post_status' => 'publish',
        'posts_per_page' => -1,
        'meta_query' => array(
            'relation' => 'OR',
...
// submit form
add_action( 'bricks/form/custom_action', 'adfoin_bricks_submission', 10, 1 );
    ...
    if ( adfoin_fs()->is__premium_only() ) {
        if ( adfoin_fs()->is_plan( 'professional', true ) ) {
            ...
            if( $uploaded_files ) {
                ...
                $posted_data[$field_id] = implode( ',', $links );
            }
        }
    }
    ...
    ?>
        <script type="text/template" id="bricks-template">
            <tr valign="top" class="alternate" v-if="trigger.formId">
                ...
                <td>
                    <p>
                        <?php esc_attr_e( 'Enable custom action in form settings. The basic AFI plugin supports text and email fields only', 'advanced-form-integration' ); ?>
                    </p>
                </td>
            </tr>
        </script>
