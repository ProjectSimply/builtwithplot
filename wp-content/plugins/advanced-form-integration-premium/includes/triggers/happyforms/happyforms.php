...
add_action( 'happyforms_submission_success', 'adfoin_happyforms_submission', 30, 2 );
...
function adfoin_happyforms_submission( $submission, $form ) {
    ...
    foreach( $submission as $key => $value ) {
        ...
        if ( adfoin_fs()->is__premium_only() ) {
            if ( adfoin_fs()->is_plan( 'professional', true ) ) {
                if( 'attachment_' == substr( $key, 0, 11 ) ) {
                    $links = array();
                    $values = maybe_unserialize( $value );
        
                    if( is_array( $values ) ) {
                        foreach( $values as $hash_id ) {
                            $post_id = $wpdb->get_var( "SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_happyforms_hash_id' AND  meta_value = '{$hash_id}'" );
        
                            if( $post_id ) {
                                $links[] = get_the_guid( intval( $post_id ) );
                            }
                        }
                    }
        
                    $posted_data[$key] = implode( ',', $links );
                    continue;
                }
