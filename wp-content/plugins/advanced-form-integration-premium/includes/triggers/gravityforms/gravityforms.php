    ...
    foreach ( $raw_fields as $field ) {
        ...
        if ( adfoin_fs()->is__premium_only() ) {
            if ( adfoin_fs()->is_plan( 'professional', true ) ) {
                if ( $field->inputs && $field->type != 'checkbox' && $field->type != 'time' && $field->type != 'consent' && $field->type != 'date' ) {
                    foreach ( $field->inputs as $input ) {
                        $fields[$input->id] = $input->label;
                    }
                    continue;
...
add_action( 'gform_after_submission', 'adfoin_gravityforms_after_submission', 10, 2 );
            ...
            if ( adfoin_fs()->is__premium_only() ) {
                if ( adfoin_fs()->is_plan( 'professional', true ) ) {
                    if( 'multiselect' == $field_type ) {
                        $multi_value          = json_decode( $value );
                        $multi_value          = implode( ',', $multi_value );
                        $posted_data[$intkey] = $multi_value;
                        $last_key             = $intkey;
                        ...
                        foreach( $entry as $searchkey => $searchvalue ) {
                            if( strpos( $searchkey, $intkey . '.' ) !== false ) {
                                $checkbox_values[] = $searchvalue;
                            } 
                        }

                        $checkbox_values = implode( ',', array_filter( $checkbox_values ) );
                        $posted_data[$intkey] = $checkbox_values;
                        $last_key             = $intkey;
                    } elseif ( 'list' == $field_type ) {
                        $posted_data[$intkey] = json_encode( unserialize( $value ) );
                        $last_key             = $intkey;
                    } elseif ( 'consent' == $field_type ) {
                        $posted_data[$intkey] = $entry[$intkey . '.1'];
                    } else{
                        $posted_data[$key] = $value;
                        $last_key          = $intkey;
                    }
