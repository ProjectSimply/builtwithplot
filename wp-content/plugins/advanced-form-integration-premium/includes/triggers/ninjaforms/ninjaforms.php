...
function adfoin_ninjaforms_after_submission( $form_data ) {
                        ...
                        if( 'date' == $field['type'] ) {
                            if( is_array( $field['value'] ) ) {
                                foreach( $field['value'] as $key => $value ) {
                                    $posted_data[$field['id'] . '_' . $key] = $value;
                                }
                            }
                        }
    ...
    if ( adfoin_fs()->is__premium_only() ) {
        if ( adfoin_fs()->is_plan( 'professional', true ) ) {
            if( isset( $form_data['extra'], $form_data['extra']['calculations'] ) ) {
                if( is_array( $form_data['extra']['calculations'] ) ) {
                    foreach( $form_data['extra']['calculations'] as $key => $value ) {
