        ...
        if ( adfoin_fs()->is__premium_only() ) {
            if ( adfoin_fs()->is_plan( 'professional', true ) ) {
                ...
                if( 'address' == $field['type'] ) {
                    $fields[$field['id'] . '-street_address'] = __( 'Street Address', 'advanced-form-integration' );
                    $fields[$field['id'] . '-address_line']   = __( 'Apartment', 'advanced-form-integration' );
                    $fields[$field['id'] . '-city']           = __( 'City', 'advanced-form-integration' );
                    $fields[$field['id'] . '-state']          = __( 'State', 'advanced-form-integration' );
                    $fields[$field['id'] . '-zip']            = __( 'ZIP', 'advanced-form-integration' );
                    $fields[$field['id'] . '-country']        = __( 'Country', 'advanced-form-integration' );
        
                    continue;
                }
        
                if( 'postdata' == $field['type'] ) {
                    $fields[$field['id'] . '-post_title']   = __( 'Post Title', 'advanced-form-integration' );
                    $fields[$field['id'] . '-post_content'] = __( 'Post Content', 'advanced-form-integration' );
                    $fields[$field['id'] . '-post_excerpt'] = __( 'Post Excerpt', 'advanced-form-integration' );
                    $fields[$field['id'] . '-post_image']   = __( 'Post Image', 'advanced-form-integration' );
                    $fields[$field['id'] . '-category']     = __( 'Post Category', 'advanced-form-integration' );
                    $fields[$field['id'] . '-post_tag']     = __( 'Post Tags', 'advanced-form-integration' );
                    
                    continue;
                }
        
                if( 'time' == $field['type'] ) {
                    $fields[$field['id'] . '-hours'] = __( 'Hours', 'advanced-form-integration' );
                    $fields[$field['id'] . '-minutes']   = __( 'Minutes', 'advanced-form-integration' );
...
add_action( 'forminator_custom_form_submit_before_set_fields', 'adfoin_forminator_submission', 999, 3 );
    ...
    if ( adfoin_fs()->is__premium_only() ) {
        if ( adfoin_fs()->is_plan( 'professional', true ) ) {
            if( is_array( $field_data_array ) ) {
                foreach( $field_data_array as $single ) {
                    if( "calculation-" == substr( $single["name"], 0, 12 ) ) {
                        $posted_data[$single["name"]] = $single["value"]["result"];
                    }

                    if( "upload-" == substr( $single["name"], 0, 7 ) ) {
                        if( isset( $single["value"]["file"] ) ) {
                            $files = $single["value"]["file"]["file_url"];

                            if( is_array( $files ) ) {
                                $files = implode(",", $files );
                            }
                            
                            $posted_data[$single["name"]] = $files;
                        }
                    }

                    if( "hidden-" == substr( $single["name"], 0, 7 ) ) {
                        $posted_data[$single["name"]] = $single["value"];
                    }

                    if( "signature-" == substr( $single["name"], 0, 10 ) ) {
                        $posted_data[$single["name"]] = $single["value"]['file']['file_url'];
                    }
                }
            }
        }
