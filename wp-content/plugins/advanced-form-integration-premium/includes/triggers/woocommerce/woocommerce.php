        ...
        if ( function_exists( "adfoin_{$action_provider}_send_data" ) ) {
            ...
            if ( adfoin_fs()->is__premium_only() ) {
                if ( adfoin_fs()->is_plan( 'professional', true ) ) {
                    ...
                    if ( 'googlesheetspro' == $action_provider ) {
                        $record_data  = json_decode( $record['data'], true );
                        $field_data   = $record_data['field_data'];
                        $wc_multi_row = ( isset( $field_data['wcMultipleRow'] ) && 'true' == $field_data['wcMultipleRow'] ) ? true : false;
    
                        if ( $wc_multi_row && isset( $item_data ) && is_array( $item_data ) ) {
                            ...
                            foreach ( $item_data as $item ) {
                                ...
                                if ( $job_queue ) {
                                    as_enqueue_async_action( "adfoin_{$action_provider}_job_queue", array(
                                        'data' => array(
                                            'record'      => $record,
                                            'posted_data' => $single_row
                                        )
                                    ) );
                                } else {
                                    call_user_func( "adfoin_{$action_provider}_send_data", $record, $single_row );
                                }
                            }
