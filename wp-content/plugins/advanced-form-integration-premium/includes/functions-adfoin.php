...
function adfoin_get_form_providers() {
    ...
    $providers = array(
        ...
        // 'breakdance'   => __( 'Breakdance Form', 'advanced-form-integration' ),
        'bricks'       => __( 'Bricks Form', 'advanced-form-integration' ),
        // 'brizy'        => __( 'Brizy Form', 'advanced-form-integration' ),
    ...
    // Add the remaining form providers to the list.
    if ( adfoin_fs()->is__premium_only() ) {
        if ( adfoin_fs()->is_plan( 'professional', true ) ) {
            $providers['elementorpro2']   = __( 'Elementor Pro Form (Manual Method)', 'advanced-form-integration' );
            $providers['webhooksinbound'] = __( 'Inbound Webhooks', 'advanced-form-integration' );
        }
    }
...
function adfoin_general_settings_view( $current_tab ) {
    ...
    <form name="general_save_form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"
        ...
        <div class="afi-row">
        <div class="col-full">
            <h3><?php _e( 'Activate Platforms', 'advacned-form-integration' ); ?></h3>
                ...
                ?>
                    ...
                    <div class="afi-elements-info">
                        <p class="afi-el-title">
                            <label for="<?php echo esc_attr($key); ?>"><?php echo esc_html( $platform['title'] ); ?></label>
                        </p>
                    </div>
                    <label class="adfoin-toggle-form form-enabled">
                    <input type="checkbox" value="1" id="<?php echo esc_attr($key); ?>" name="platforms[<?php echo esc_attr($key); ?>]" <?php checked( $status, 1 ); ?>>
            ...
            <h3><?php _e( 'General Settings', 'advacned-form-integration' ); ?></h3>
            <div class="afi-checkbox-container">
                <div class="afi-checkbox">
                    <div class="afi-elements-info">
                        <p class="afi-el-title">
                            <label for="adfoin_disable_log"><?php _e( 'Disable Log', 'advanced-form-integration' ); ?></label>
                        </p>
                    </div>
                    <label class="adfoin-toggle-form form-enabled">
                ...
                <div class="afi-checkbox">
                    <div class="afi-elements-info">
                        <p class="afi-el-title">
                            <label for="adfoin_disable_st"><?php _e( 'Disable Special Tags', 'advanced-form-integration' ); ?></label>
                        </p>
                    </div>
                    <label class="adfoin-toggle-form form-enabled">
                ...
                <div class="afi-checkbox">
                    <div class="afi-elements-info">
                        <p class="afi-el-title">
                            <label for="adfoin_enable_utm"><?php _e( 'Send UTM Variables', 'advanced-form-integration' ); ?></label>
                        </p>
                    </div>
                    <label class="adfoin-toggle-form form-enabled">
                ...
                <div class="afi-checkbox">
                    <div class="afi-elements-info">
                        <p class="afi-el-title">
                            <label for="adfoin_job_queue"><?php _e( 'Enable Job Queue', 'advanced-form-integration' ); ?></label>
                        </p>
                    </div>
                    <label class="adfoin-toggle-form form-enabled">
 ...
 * @return void
 */
function adfoin_add_to_log( $return, $url, $args, $record, $log_id = '' ) {
    $log_settings = get_option( 'adfoin_general_settings_log' ) ? get_option( 'adfoin_general_settings_log' ) : '';
...
function adfoin_capture_utm_and_url_values(){
	...
	foreach ($fields as $field => $title ) {
		...
		setcookie( $field, $cookie_fields[$field], time()+60*60*24*30, '/', $domain );
