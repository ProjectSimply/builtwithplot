...
function adfoin_benchmark_settings_view( $current_tab ) {
    ...
    <form name="benchmark_save_form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"
            ...
            <tr valign="top">
                ...
                <td>
                    <input type="text" name="adfoin_benchmark_api_key"
                           ...
                           class="regular-text"/>
                    <p class="description" id="code-description"><?php _e( 'Go to Profile > Integrations > API Key and copy the key', 'advanced-form-integration' ); ?></a></p>
                </td>
            </tr>
        </table>
        <?php submit_button(); ?>
...
function adfoin_benchmark_request( $endpoint, $method = 'GET', $data = array(), $record = array() ) {
...
function adfoin_benchmark_create_contact( $list_id, $properties, $record ) {
    ...
    $response = adfoin_benchmark_request( $endpoint, 'POST', $properties, $record);
    
    return $response;
}

function adfoin_benchmark_update_contact(  $list_id, $contact_id, $properties, $record ) {
    $endpoint = "Contact/{$list_id}/ContactDetails/{$contact_id}";
    $response = adfoin_benchmark_request( $endpoint, 'PATCH', $properties, $record);
...
function adfoin_benchmark_send_data( $record, $posted_data ) {
    ...
    if( $task == "subscribe" ) {
        ...
        if($contact_id){
            $return = adfoin_benchmark_update_contact( $list_id, $contact_id, $data, $record );
        }else{
