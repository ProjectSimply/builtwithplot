...
function adfoin_sendx_settings_view( $current_tab ) {
    ...
    <form name="sendx_save_form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"
        ...
        <table class="form-table">
            <tr valign="top">
                <th scope="row"> <?php _e( 'Team ID', 'advanced-form-integration' ); ?></th>
                <td>
                    <input type="text" name="adfoin_sendx_team_id"
                           value="<?php echo esc_attr( $team_id ); ?>" placeholder="<?php _e( 'Enter Team ID', 'advanced-form-integration' ); ?>"
                           class="regular-text"/>
                    <p class="description" id="code-description"><?php _e( 'Go to <a target="_blank" rel="noopener noreferrer" href="https://app.sendx.io/setting">SendX settings page</a> and scroll down to the bottom.', 'advanced-form-integration' ); ?></a></p>
                </td>
            </tr>
            <tr valign="top">
                ...
                if ( adfoin_fs()->is__premium_only() ) {
                    if ( adfoin_fs()->is_plan( 'professional', true ) ) {
                        ...
                        <tr valign="top" v-if="action.task == 'subscribe'">
                            ...
                            <td scope="row">
                                <span><?php printf( __( 'To use custom fields, please create a <a href="%s">new integration</a> and select SendX [PRO] in the action field.', 'advanced-form-integration' ), admin_url('admin.php?page=advanced-form-integration-new') ); ?></span>
                            </td>
                        </tr>
                        <?php
                    }
 ...
 * SendX API Request
 */
function adfoin_sendx_request($endpoint, $method = 'GET', $data = array(), $record = array())
{
