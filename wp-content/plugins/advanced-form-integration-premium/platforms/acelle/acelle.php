		...
		<div id="post-body" class="metabox-holder columns-2">
			...
			<div id="post-body-content">
				<div class="meta-box-sortables ui-sortable">
						...
						<h2 class="hndle"><span><?php esc_attr_e( 'Acelle Accounts', 'advanced-form-integration' ); ?></span></h2>
						<div class="inside">
                            <div id="acelle-auth">
                                ...
                                <table v-if="tableData.length > 0" class="wp-list-table widefat striped">
                                    <thead>
                                        <tr>
                                            ...
                                            <th><?php _e('API Ednpoint', 'advanced-form-integration'); ?></th>
                                ...
                                <form @submit.prevent="addOrUpdateRow">
                                    <table class="form-table">
                                        ...
                                        <tr valign="top">
                                            <th scope="row"> <?php _e( 'API Endpoint', 'advanced-form-integration' ); ?></th>
                                            <td>
                ...
                if ( adfoin_fs()->is__premium_only() ) {
                    if ( adfoin_fs()->is_plan( 'professional', true ) ) {
                        ...
                        <tr valign="top" v-if="action.task == 'subscribe'">
                            ...
                            <td scope="row">
                                <span><?php printf( __( 'To use the Pro features, please create a <a href="%s">new integration</a> and select Acelle Mail [PRO] in the action field.', 'advanced-form-integration' ), admin_url('admin.php?page=advanced-form-integration-new') ); ?></span>
                            </td>
                        </tr>
                        <?php
                    }
 ...
 */
function adfoin_acelle_request( $endpoint, $method = 'GET', $data = array(), $record = array(), $cred_id = '' ) {
...
function adfoin_acelle_send_data( $record, $posted_data ) {
    ...
    if( $task == 'subscribe' ) {
        ...
        if( isset( $data['FIRST_NAME'] ) && $data['FIRST_NAME'] ) { $subscriber_data['FIRST_NAME'] = adfoin_get_parsed_values( $data['FIRST_NAME'], $posted_data ); }
        if( isset( $data['LAST_NAME'] ) && $data['LAST_NAME'] ) { $subscriber_data['LAST_NAME'] = adfoin_get_parsed_values( $data['LAST_NAME'], $posted_data ); }
