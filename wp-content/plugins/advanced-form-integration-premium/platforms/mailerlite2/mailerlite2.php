...
function adfoin_mailerlite2_settings_view( $current_tab ) {
    ...
    <form name="mailerlite2_save_form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"
        ...
        <table class="form-table">
            <tr valign="top">
                <th scope="row"> <?php _e( 'MailerLIte API Token', 'advanced-form-integration' ); ?></th>
                <td>
                    <input type="text" name="adfoin_mailerlite2_api_key"
                           value="<?php echo esc_attr( $api_key ); ?>" placeholder="<?php _e( 'Enter API Token', 'advanced-form-integration' ); ?>"
                           class="regular-text"/>
                    <p class="description" id="code-description"><?php _e( 'Please go to Integrations > API and generate a new token', 'advanced-form-integration' ); ?></a></p>
                </td>
            </tr>
        </table>
        <?php submit_button(); ?>
...
add_action( 'admin_post_adfoin_save_mailerlite2_api_key', 'adfoin_save_mailerlite2_api_key', 10, 0 );
                ...
                if ( adfoin_fs()->is__premium_only() ) {
                    if ( adfoin_fs()->is_plan( 'professional', true ) ) {
                        ...
                        <tr valign="top" v-if="action.task == 'subscribe'">
                            ...
                            <td scope="row">
                                <span><?php printf( __( 'To use the Pro features, please create a <a href="%s">new integration</a> and select MailerLite [PRO] in the action field.', 'advanced-form-integration' ), admin_url('admin.php?page=advanced-form-integration-new') ); ?></span>
                            </td>
                        </tr>
                        <?php
                    }
