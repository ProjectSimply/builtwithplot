...
class ADFOIN_LionDesk extends Advanced_Form_Integration_OAuth2 {
    ...
    private function __construct() {
        ...
        add_action( 'wp_ajax_adfoin_get_liondesk_list', array( $this, 'get_liondesk_list' ), 10, 0 );
                ...
                if ( adfoin_fs()->is__premium_only() ) {
                    if ( adfoin_fs()->is_plan( 'professional', true ) ) {
                        ...
                        <tr valign="top" v-if="action.task == 'add_contact'">
                            ...
                            <td scope="row">
                                <span><?php printf( __( 'To use the Pro features, please create a <a href="%s">new integration</a> and select LionDesk [PRO] in the action field.', 'advanced-form-integration' ), admin_url('admin.php?page=advanced-form-integration-new') ); ?></span>
                            </td>
                        </tr>
                        <?php
                    }
    ...
    public function create_contact( $properties, $record = array() ) {
        $response = $this->liondesk_request( 'contacts', 'POST', $properties, $record );
