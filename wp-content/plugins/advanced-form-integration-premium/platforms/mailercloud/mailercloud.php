...
function adfoin_mailercloud_settings_view( $current_tab ) {
    ...
    <form name="mailercloud_save_form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"
            ...
            <tr valign="top">
                ...
                <td>
                    <input type="text" name="adfoin_mailercloud_api_key"
                           value="<?php echo esc_attr( $api_key ); ?>" placeholder="<?php _e( 'Please enter the API Key', 'advanced-form-integration' ); ?>"
                           class="regular-text"/>
                    <p class="description" id="code-description"><?php _e( 'Go to Profile > Account > Integrations > API Integrations. Create an API key', 'advanced-form-integration' ); ?></p>
                </td>
            </tr>
        </table>
        <?php submit_button(); ?>
...
add_action( 'admin_post_adfoin_mailercloud_save_api_key', 'adfoin_save_mailercloud_api_key', 10, 0 );
...
function adfoin_get_mailercloud_list() {
    ...
    $lists = array();
    $limit = 100;
    $data  = adfoin_mailercloud_request( 'lists/search', 'POST', array( 'limit' => $limit, 'page' => 1 ) );
    ...
    if( !is_wp_error( $data ) ) {
        ...
        if( $pagination_needed >= 2) {
            ...
            for( $i = 2; $i <= $pagination_needed; $i++ ){
                $response_pages[$i] = adfoin_mailercloud_request( 'lists/search', 'POST', array( 'limit' => $limit, 'page' => $i ) );
