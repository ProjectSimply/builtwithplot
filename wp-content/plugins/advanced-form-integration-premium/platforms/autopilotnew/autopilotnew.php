...
function adfoin_autopilotnew_settings_view( $current_tab ) {
    ...
    <form name="autopilotnew_save_form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"
            ...
            <tr valign='top'>
                ...
                <td>
                    <select name="adfoin_autopilotnew_data_center" id="adfoin-autopilotnew-data-center">
                        <option value="us" <?php selected( $data_center, 'us' ); ?>>US</option>
                        <option value="eu" <?php selected( $data_center, 'eu' ); ?>>EU</option>
                        <option value="au" <?php selected( $data_center, 'au' ); ?>>AU</option>
                    </select>
                </td>
            </tr>
            <tr valign="top">
                <th scope="row"> <?php _e( 'Private Key', 'advanced-form-integration' ); ?></th>
                <td>
                    <input type="text" name="adfoin_autopilotnew_api_key"
                           value="<?php echo esc_attr( $api_key ); ?>" placeholder="<?php _e( 'Enter Private Key', 'advanced-form-integration' ); ?>"
                           class="regular-text"/>
                    <p class="description" id="code-description"><?php _e( '1. Create a new data source.<br> 2. Select Custom API.<br> 3. Put a name.<br> 4. Copy private key.<br> 5. Save data source. ', 'advanced-form-integration' ); ?></a></p>
                </td>
            </tr>
        </table>
        <?php submit_button(); ?>
                ...
                if ( adfoin_fs()->is__premium_only() ) {
                    if ( adfoin_fs()->is_plan( 'professional', true ) ) {
                        ...
                        <tr valign="top" v-if="action.task == 'subscribe'">
                            ...
                            <td scope="row">
                                <span><?php printf( __( 'To use the Pro features, please create a <a href="%s">new integration</a> and select Autopilot [PRO] in the action field.', 'advanced-form-integration' ), admin_url('admin.php?page=advanced-form-integration-new') ); ?></span>
                            </td>
                        </tr>
                        <?php
                    }
...
function adfoin_autopilotnew_send_data( $record, $posted_data ) {
    ...
    if( $task == 'subscribe' ) {
        ...
        $return = adfoin_autopilotnew_request( 'person/merge', 'POST', $data, $record );
    }
...
// Ortto API Call
function adfoin_autopilotnew_request( $endpoint, $method = 'GET', $data = array(), $record = array() ) {
