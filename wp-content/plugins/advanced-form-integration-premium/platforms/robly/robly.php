...
function adfoin_robly_settings_view( $current_tab ) {
    ...
    <form name="robly_save_form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"
        ...
        <table class="form-table">
            <tr valign="top">
                <th scope="row"> <?php _e( 'API ID', 'advanced-form-integration' ); ?></th>
                <td>
                    <input type="text" name="adfoin_robly_api_id"
                           value="<?php echo esc_attr( $api_id ); ?>" placeholder="<?php _e( 'Please enter the API ID', 'advanced-form-integration' ); ?>"
                           class="regular-text"/>
                    <p class="description" id="code-description"><?php _e( 'Go to Settings > API Details to get API ID and API Key', 'advanced-form-integration' ); ?></a></p>
                </td>
            </tr>
            <tr valign="top">
                ...
                if ( adfoin_fs()->is__premium_only() ) {
                    if ( adfoin_fs()->is_plan( 'professional', true ) ) {
                        ...
                        <tr valign="top" v-if="action.task == 'subscribe'">
                            ...
                            <td scope="row">
                                <span><?php printf( __( 'To use the Pro features, please create a <a href="%s">new integration</a> and select Robly [PRO] in the action field.', 'advanced-form-integration' ), admin_url('admin.php?page=advanced-form-integration-new') ); ?></span>
                            </td>
                        </tr>
                        <?php
                    }
 ...
 * Robly API Request
 */
function adfoin_robly_request( $endpoint, $method = 'GET', $data = array(), $record = array() ) {
...
function adfoin_robly_send_data( $record, $posted_data ) {
    ...
    if( $task == 'subscribe' ) {
        ...
        if( $contact_id ) {
            $return = adfoin_robly_request( 'contacts/update_full_contact?member_id=' . $contact_id, 'POST', $data, $record );
        } else{
            $return = adfoin_robly_request( 'sign_up/generate', 'POST', $data, $record );
        }
    }
}